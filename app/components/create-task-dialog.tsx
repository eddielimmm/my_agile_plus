...
interface CreateTaskDialogProps {
  isOpen: boolean
  onClose: () => void
  onCreateTask: (task: Omit<Task, "id" | "isCompleted" | "timeSpent" | "createdAt">) => void
...
export function CreateTaskDialog({ isOpen, onClose, onCreateTask, folders }: CreateTaskDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[450px] bg-white dark:bg-gray-800 p-6" aria-describedby="dialog-description">
        <DialogHeader className="mb-4">
          <DialogTitle className="text-xl font-bold bg-gradient-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent dark:text-white">
            Create New Task
          </DialogTitle>
          <DialogDescription id="dialog-description">Fill in the details below to create a new task.</DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              required
              className="h-10 border-2 focus:ring-2 focus:ring-purple-500 dark:bg-gray-700 dark:text-white dark:border-gray-600"
            />
          </div>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    className={cn(
                      "w-full h-10 justify-start text-left font-normal border-2 dark:bg-gray-700 dark:text-white dark:border-gray-600",
                ...
                <PopoverContent className="w-auto p-0 bg-white dark:bg-gray-800">
                  <Calendar
                    mode="single"
                    selected={dueDate}
                    onSelect={(newDate) => setDueDate(newDate || new Date())}
                    initialFocus
